...
Route::group(['prefix' => 'basts', 'middleware' => ['auth']], function () {

    Route::get('/', [BastController::class, 'index'])
        ->name('basts.index')
        ->breadcrumbs(fn (Trail $trail) =>
            $trail->parent('home')
                ->push('BAST', route('basts.index')));
    ...
    Route::get('/{bast}', [BastController::class, 'show'])
        ->name('basts.show')
        ->breadcrumbs(fn (Trail $trail, $bast) =>
            $trail->parent('basts.index')
                ->push('Detail BAST', route('basts.show', $bast)));

    Route::get('/{bast}/edit', [BastController::class, 'edit'])
        ->name('basts.edit')
        ->breadcrumbs(fn (Trail $trail, $bast) =>
            $trail->parent('basts.show', $bast)
                ->push('Edit Draft BAST', route('basts.edit', $bast)));

    Route::put('/{bast}', [BastController::class, 'update'])
        ->name('basts.update');

    Route::post('/{bast}/finalize', [BastController::class, 'finalize'])
        ->name('basts.finalize');
